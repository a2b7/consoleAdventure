...
class Enemy:
    ...
    def __init__(self, enemies_killed, dicts=dict_class):
        state = r.sample(dicts.states, 1)[0]
        if enemies_killed <= 15:
            self.name = state + ' ' + r.sample(dicts.names_grade1, 1)[0]
            tier = 1
        elif enemies_killed <= 30:
            self.name = state + ' ' + r.sample(dicts.names_grade2, 1)[0]
            tier = 2
        elif enemies_killed <= 45:
            self.name = state + ' ' + r.sample(dicts.names_grade3, 1)[0]
            tier = 3
        elif enemies_killed <= 60:
            self.name = state + ' ' + r.sample(dicts.names_grade4, 1)[0]
            tier = 4
        elif enemies_killed <= 75:
            self.name = state + ' ' + r.sample(dicts.names_grade5, 1)[0]
            tier = 5
        elif enemies_killed <= 90:
            self.name = state + ' ' + r.sample(dicts.names_grade6, 1)[0]
            tier = 6
        else:
            self.name = state + ' ' + r.sample(dicts.names_grade7, 1)[0]
            tier = 7
        ...
        text = str(r.sample(dicts.messages, 1)[0].replace('%enemy%', self.name))
